...
function placeBarriers() {
    if (!loopEnd_barriers) {
        ...
        barrier_loop = setTimeout(placeBarriers);
    }
}
...
function onClickPlaceBarriers(e) {
    for(i = 0; i < 1200; i+= (1200 / cols)) {
        if (e.clientX > i + 10 && e.clientX < i + (1200 / cols) + 10) {
            barrierX = i / (1200 / cols) + 1
        }
    }
    for (j = 0; j < 800; j+= (800 / rows)) {
        if (e.clientY > j + 140 && e.clientY < j + (800 / rows) + 140) {
            barrierY = j / (800 / rows) + 1
        }
    }

    ctx_pg.beginPath();
    ctx_pg.fillStyle = "black";
    ctx_pg.fillRect(barrierX * (1200 / cols) - (1200 / cols) + 1, barrierY * (800 / rows) - (800 / rows) + 1, 1200 / cols - 2, 800 / rows - 2);
    ctx_pg.stroke();
}
